...
$('.slider-wrapper .HTML .widget-content').each(function () {
  ...
  function buildSlider(data, matchType) {
    var url = "";
    ...
    featcode += '</ul><div id="slider-nav"/>' + (matchType === 'label' ? '<span id="feat-star-bg"/><span id="feat-star"/>' : '') + '</div>';

    $('.slider-wrapper .HTML .widget-content').each(function () {
      if ($(this).find("div").attr("class").match(matchType)) {
        $(this).html(featcode);
        $(this).removeClass('widget-content').addClass('slider-content');

        // Inisialisasi FlexSlider jika plugin tersedia
